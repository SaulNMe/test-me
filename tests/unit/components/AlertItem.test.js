...
describe('rendering', () => {
	...
	describe('when HugeText is true', () => {
		beforeEach(() => {
			wrapper = createWrapper({ percent: false });
		});
	...
	describe('when props are not set', () => {
		...
		describe('when HugeText is true', () => {
			beforeEach(() => {
				wrapper = createWrapper({ seen: true });
			});
			it('displays default backgroundColor', () => {
				expect(wrapper.find(TouchableOpacity).props().style.backgroundColor).toEqual(Colors.lighterGreen);
			});
		});
	});
})
